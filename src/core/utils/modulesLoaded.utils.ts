...
export function modulesLoadedUtils(allAppRoutes: express.Router[], dbConChecker: () => void) {
    ...
    typeof allAppRoutes == "function" ? console.log(`${colors.whiteBright(`  content`)} ${colors.green('Kernel :')} ${colors.cyan(`${colors.bold(`Routers service`)} `)} has been loaded successfully ${colors.green(`✔`)}`) : "";
