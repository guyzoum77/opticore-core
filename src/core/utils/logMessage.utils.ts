...
export class LogMessageUtils {
    ...
    static error(title: string, action: any, typeActionTitle: any, typeActionDescribe: any,
        ...
        console.log(`${colors.red(`✘`)} ${colors.bgRed(`[ ${colors.white(`${colors.bold(`${title}`)}`)} ]`)} | ${dateTimeFormattedUtils()} | [ ${colors.red(`${colors.bold(`${action}`)}`)} ] | [ ${colors.bold(`${typeActionTitle}`)} ] ${colors.red(`${typeActionDescribe}`)} - [ ${colors.bold(`${messageTitle}`)} ] ${colors.red(`${messageContent}`)} - [ ${colors.red(`${colors.bold(`HttpCode`)}`)} ] ${colors.red(`${colors.bold(`${httpCodeValue}`)}`)} `);
    }
}